...
#if ENABLE_INPUT_SYSTEM
        if (Event.current.type == EventType.MouseUp || Mouse.current.leftButton.wasReleasedThisFrame)
            mouseState = MouseState.Up;
        else if (Event.current.type == EventType.MouseDown || Mouse.current.leftButton.wasPressedThisFrame)
